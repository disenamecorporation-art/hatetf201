        ...
        >
          <div className="text-center mb-12">
            ...
            <h1 className="text-4xl font-serif tracking-tight text-deep-black">
              Iniciar <span className="italic">Sesión</span>
            </h1>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-8">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full py-5 bg-deep-black text-bone uppercase tracking-[0.3em] font-bold hover:bg-gold-dark transition-all duration-500 disabled:opacity-50 group flex items-center justify-center gap-3"
            >
              {loading ? 'Procesando...' : 'Entrar'} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
            </button>
          </form>
          ...
          <div className="mt-12 text-center">
            <p className="text-sm text-deep-black/40 font-light">
              ¿No tiene una cuenta?{' '}
              <Link to="/registro" className="text-gold-dark font-bold hover:underline">Solicitar Registro</Link>
            </p>
          </div>
        </motion.div>
      </section>
